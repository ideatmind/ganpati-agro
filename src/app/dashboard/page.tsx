...
interface DashboardData {
  roles: string[]; earnedPaise: number; paidPaise: number; availablePaise: number; referralCode: string;
...
function date(value: string) { return new Intl.DateTimeFormat("mr-IN", { dateStyle: "medium" }).format(new Date(value)); }
...
export default async function DashboardPage() {
  ...
  const referralUrl = `${origin}/register?ref=${encodeURIComponent(data.referralCode)}`;

  return <main className="dashboard-shell">
    <header className="dashboard-header"><div><span className="eyebrow">नमस्कार</span><h1>{identity.displayName}</h1><p>{identity.roles.join(" · ")}</p></div><DashboardActions referralUrl={referralUrl} /></header>
    <section className="referral-strip"><div><span>आपला रेफरल कोड</span><strong>{data.referralCode}</strong><small>{referralUrl}</small></div><Link className="button button-gold" href={`/register?ref=${data.referralCode}`}>नवीन नोंदणी</Link></section>
    <section className="metric-grid">
      <article><span>एकूण कमाई</span><strong>{formatRupees(data.earnedPaise)}</strong></article>
