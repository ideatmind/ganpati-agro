...
interface Receipt { membershipType:MembershipType; receiptNumber: string; registrationReference: string; membershipNumber: string; memberName: string; memberMobile: string; amountPaise: number; currency: string; paymentId: string; issuedAt: string }
...
export default async function ReceiptPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return <main className="receipt-page"><div className="receipt-actions"><Link className="button button-outline" href="/dashboard">My dashboard</Link><Link className="button button-outline" href="/">Website</Link><PrintReceiptButton /></div><article className="receipt-card">
    ...
    <dl><div><dt>Membership type</dt><dd>{MEMBERSHIP_LABELS[receipt.membershipType]}</dd></div><div><dt>Receipt number</dt><dd>{receipt.receiptNumber}</dd></div><div><dt>Member</dt><dd>{receipt.memberName}</dd></div><div><dt>Mobile</dt><dd>{receipt.memberMobile}</dd></div><div><dt>Registration</dt><dd>{receipt.registrationReference}</dd></div><div><dt>Membership</dt><dd>{receipt.membershipNumber}</dd></div><div><dt>Razorpay payment</dt><dd>{receipt.paymentId}</dd></div></dl>
