...
## UI consistency and security review — 25 September 2026

Implemented and verified on codex/consistent-ui-security: shared public/form/dashboard/admin colors and controls, the unpublished group-participation homepage section with only the ₹500 CTA, preserved local setup/tools, and a shared registration/login account-rate limit. Exclusive remains available through navigation and the form switch. No database migration is needed.

Passed lint, typecheck, 41 application tests, production build, nine SQL suites, 12-way payment/deletion concurrency, HTTP payment/authentication journeys, four-role responsive browser checks, membership/form navigation checks and solid-color text contrast checks on 13 admin and five public pages. Dependency audit reports zero known vulnerabilities; source/client secret scan found no configured private values. The detailed scope and evidence are in docs/UI_SECURITY_REVIEW_20260925.md.

The dependency setup is complete: Node 22.23.3, locked npm dependencies and self-hosted fonts. Normal application work uses the existing hosted Supabase; local PostgreSQL is used only for isolated security/payment tests. Earlier dated status sections below record historical states and are superseded by the newest release notes.
...
Migration 20260925122350_focused_value_chain_membership.sql is additive for existing identity and financial data, preserves legacy RPC compatibility, and enforces one membership per type. A populated PostgreSQL 16 rehearsal preserved original records and pending checkout retries. Deployed routine bodies match the repository baseline. Applied to hosted Supabase as version 20260925122350. Before/after aggregate snapshots across 11 identity/payment/plot tables were identical; both fee RPCs return the expected 50000/250000 paise. All five new RPCs remain service-role-only and every public table retains RLS. Advisors report informational deny-by-default RLS/unused-index notices only. The feature is committed on codex/focused-value-chain in the managed focused-value-chain worktree. The owner reconnected terminal Git as ideatmind and pushed the branch; the earlier GitHub publishing-access blocker is resolved. PR #11 was merged as 2752e85654109c67904d31be81ed5a01e8eda0d9 and production deployment was verified.

Passed lint, typecheck, 41 application tests, production build, nine database suites, isolated HTTP payment journeys and 12-way concurrent checkout/capture/deletion. Browser checks cover 320/390/1280px forms, exact crop options, keyboard controls, navigation, prices, admin filtering and existing-member prefill without exposing Aadhaar. The current production homepage, default registration and login still pass www-to-apex redirect/HTTP-200 checks after the migration. All payment fixtures run against a disposable loopback database with a stubbed provider; no real payment is charged. A local database is not required to run the application against hosted Supabase.

Navigation follow-up: the homepage’s “नोंदणी करा” opens the standard ₹500 form. Both forms show a visible “Exclusive form” switch for the ₹2,500 form, with the same label in website and employee/admin navigation. Demo-data fill controls are absent from both forms. The navigation update passed lint, typecheck, all 41 application tests, production build and mobile/desktop browser checks. The published production navigation was verified.
