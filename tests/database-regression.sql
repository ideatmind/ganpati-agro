...
begin
  ...
  denied:=false;
  begin perform public.finalize_registration_payment('order_audit1','pay_audit1',100,'INR','employee',null);exception when others then denied:=true;end;
  ...
  result:=public.record_payment_event('evt_audit1','payment.captured','order_audit1','pay_audit1',100,'INR','{}');
  assert result?'receiptToken','Webhook did not finalize payment';
  retry:=public.finalize_registration_payment('order_audit1','pay_audit1',100,'INR','farmer',true);
  assert retry->>'receiptToken'=result->>'receiptToken','Browser/webhook deduplication failed';
  perform public.record_payment_event('evt_audit1','payment.captured','order_audit1','pay_audit1',100,'INR','{}');
  perform public.record_payment_event('evt_audit2','order.paid','order_audit1','pay_audit1',100,'INR','{}');
  ...
  assert (select amount_paise from public.referral_earnings where registration_id=reg_id)=10,'Incorrect commission';
  ...
  result:=public.finalize_registration_payment('order_audit1','pay_extra',100,'INR','farmer',true);
  assert result->>'paymentException'='true','Additional captured payment silently dropped';
  perform public.finalize_registration_payment('order_audit1','pay_extra',100,'INR','farmer',true);
  ...
  result:=public.record_payment_event('evt_early','payment.captured','order_early','pay_early',100,'INR','{}');
  ...
  result:=public.record_payment_event('evt_failed_late','payment.failed','order_early','pay_early',100,'INR','{}');
  ...
  result:=public.record_payment_event('evt_early','payment.captured','order_early','pay_changed',100,'INR','{}');
  assert result->>'failed'='true','Event ID accepted with different payment';

  payout:=public.record_offline_payout(manager,profile,1,'cash','audit-ref','test',now(),key);
  retry:=public.record_offline_payout(manager,profile,1,'cash','audit-ref','test',now(),key);
  assert payout->>'id'=retry->>'id','Payout retry deducted twice';
  denied:=false;
  begin perform public.record_offline_payout(manager,profile,2,'cash','audit-ref','test',now(),key);exception when others then denied:=true;end;
  ...
  denied:=false;
  begin update public.referral_payouts set amount_paise=2 where id=(payout->>'id')::uuid;exception when others then denied:=true;end;
  ...
  perform public.record_payment_event('audit-refund','refund.processed',null,'pay_audit1',100,'INR','{}');
