...
test("the registration fee is stored in paise", () => {
  assert.equal(REGISTRATION_FEE_PAISE, 50_000);
  assert.match(formatRupees(REGISTRATION_FEE_PAISE), /500/);
});

test("the default referral earning is ten percent of the registration fee", () => {
  const basisPoints = 1_000;
  assert.equal(REGISTRATION_FEE_PAISE * basisPoints / 10_000, 5_000);
});
