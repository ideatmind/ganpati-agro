...
export async function RegistrationPage({type,searchParams}:{type:MembershipType;searchParams:Promise<{ref?:string|string[]}>}){
 ...
 <nav className="membership-switch" aria-label="Choose membership form">{MEMBERSHIP_TYPES.map(item=><Link key={item} href={membershipPath(item)+(referral?'?ref='+encodeURIComponent(referral):'')} aria-current={item===type?'page':undefined}><strong>{item==='standard'?'नोंदणी करा':'Exclusive form'}</strong><span>{item==='standard'?'₹500 · नियमित सभासदत्व':'₹2,500 · केंद्रित मूल्य साखळी सभासदत्व'}</span></Link>)}</nav>
