...
async function executeRpc<T>(name:string,params:Record<string,unknown>):Promise<T>{
  ...
  if (!response.ok) {
    ...
    const safe = new Map<string, readonly [number,string,string]>([
      ...
      ["Sign in to add another membership", [401,"MEMBER_LOGIN_REQUIRED","Sign in with the existing member account, then reopen this membership form."]],
      ["Existing member details must match", [409,"MEMBER_DETAILS_CHANGED","Use your saved account details. Ask a manager to update your profile if needed."]],
      ["Select all memberships before permanent deletion", [409,"SHARED_PROFILE","Move all of this person's membership registrations to Trash and select them together before permanent deletion."]],
