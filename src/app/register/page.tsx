...
import { getCheckoutId } from "@/features/registration/server/checkout";
...
export default async function RegisterPage({ searchParams }: { searchParams: Promise<{ ref?: string | string[] }> }) {
  const [query, identity, checkoutId] = await Promise.all([searchParams, getIdentity(),getCheckoutId()]);
