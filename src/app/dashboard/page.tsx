...
interface DashboardData {
  ...
  pendingOnboardings:{reference:string;farmerName:string;amountPaise:number;paymentMode:string;status:string}[]; recentEarnings: Earning[]; recentPayouts: Payout[]; recentOnboardings: Onboarding[];
}
...
export default async function DashboardPage({searchParams}:{searchParams:Promise<{personal?:string}>}) {
  ...
  return <main className="dashboard-shell">
    ...
    <nav className="dashboard-quick-links" aria-label="Dashboard navigation">{isOps&&<Link href="/dashboard/admin">Operations workspace</Link>}{isEmployee&&<><Link href="/register">Add registration</Link><Link href="#pending-work">Pending work</Link><Link href="#my-farmers">My farmers</Link></>}{data.referralCode&&<Link href="#my-referrals">My referrals</Link>}<Link href="#my-password">Account &amp; password</Link><Link href="/">Website</Link></nav>
