...
const guidance:Record<string,string>={
 ...
 crop_names:'Select at least one crop from the selected cluster, without duplicates.',area_acres:'Enter acres from 0.01 to 100,000, using at most two decimal places.',
 plot_no:'Enter a plot/survey number of up to 100 characters.',irrigation_sources:'Select at least one irrigation source from the list, without duplicates.',
...
export function validationFeedback(error:ZodError){
 const path=error.issues[0]?.path??[];const fieldPath=typeof path.at(-1)==='number'?path.slice(0,-1):path;const key=String(fieldPath.at(-1)??'');
 // Never reflect validator messages, submitted values or arbitrary object keys.
 return guidance[key]?{error:guidance[key],field:fieldPath.map(String).join('.')}: {error:'Please check the submitted fields.'};
}
