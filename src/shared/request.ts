export class RequestError extends Error {
  ...
  constructor(message:string,field?:string,code?:string){super(message);this.field=field;this.code=code;}
}
export async function requestJson<T>(url: string, body?: unknown, method = "POST"): Promise<T> {
  ...
  try {
    ...
    const result = await response.json().catch(() => ({})) as T & { error?: string;field?:string;code?:string };
    if (!response.ok) throw new RequestError(result.error || "Request could not be completed.",result.field,result.code);
    return result;
  } catch (error) {
