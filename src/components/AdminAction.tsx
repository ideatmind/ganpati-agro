...
export function AdminAction({label,endpoint,body,method='PATCH',confirmation}:{label:string;endpoint:string;body:Record<string,string>;method?:string;confirmation?:string}){
 ...
 return <div className="inline-action"><button type="button" className="admin-button admin-button-secondary" disabled={busy} onClick={()=>void run()}>{busy?'Processing…':label}</button>{message&&<p role="status">{message}</p>}</div>;
}
